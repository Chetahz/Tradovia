...
export function LandingPage() {
  return <div className="landing-page">
    <header className="landing-header">
      <Link className="brand" href="/">TRADOVIA</Link>
    ...
    <main>
      <section className="landing-hero">
        <div className="hero-copy"><p className="eyebrow">TRADING INTELLIGENCE PLATFORM</p><h1>Know your trading.<br/><span>Build your edge.</span></h1><p>Tradovia helps traders transform trade history into clear performance patterns, better reviews and more informed decisions.</p><div className="hero-actions"><Link className="cta-primary" href="/workspace">Start with Tradovia</Link><a className="cta-secondary" href="#product">Explore the platform</a></div><small>Thailand first · Built for Web · Mobile-ready architecture</small></div>
        <Card className="product-preview"><div className="preview-top"><span>Performance Overview</span><span className="data-state">Illustrative preview</span></div><div className="preview-metrics"><div><span>Net P&L</span><strong>—</strong></div><div><span>Win Rate</span><strong>—</strong></div><div><span>Realized R</span><strong>—</strong></div></div><div className="preview-chart"><div className="chart-line"/><span>Your real performance appears after you add or import trades.</span></div><div className="preview-insight"><span>TRADOVIA INTELLIGENCE</span><strong>Result → Why → Action</strong><p>Insights will always be tied back to your own trading data.</p></div></Card>
      </section>
      ...
      <section id="product" className="landing-section"><div className="section-intro"><p className="eyebrow">ONE WORKSPACE</p><h2>From recording trades to understanding them.</h2><p>Designed to keep the daily workflow simple while making the accumulated data more useful over time.</p></div><div className="product-grid">{productAreas.map(([title,copy])=><Card key={title} className="product-card"><span className="product-index">0{productAreas.findIndex(x=>x[0]===title)+1}</span><h3>{title}</h3><p>{copy}</p></Card>)}</div></section>
      <section id="intelligence" className="landing-section intelligence-section"><div><p className="eyebrow">TRADOVIA INTELLIGENCE</p><h2>Not more data.<br/>Better understanding.</h2></div><Card className="intelligence-card"><div><span>RESULT</span><strong>What happened?</strong></div><div><span>WHY</span><strong>What patterns contributed?</strong></div><div><span>ACTION</span><strong>What should you review next?</strong></div><p>No Buy/Sell signals. Intelligence is designed to help users understand their own performance.</p></Card></section>
      <section id="pricing" className="landing-section"><div className="section-intro"><p className="eyebrow">PLANS</p><h2>Start simple. Upgrade when your workflow needs more.</h2></div><div className="pricing-grid"><Card className="price-card"><span>Starter</span><strong>Free</strong><p>Core manual journal, calendar and basic analytics.</p></Card><Card className="price-card featured"><span>Pro</span><strong>฿199 <small>/ month</small></strong><p>Unlimited journal, full analytics and Edge Intelligence.</p></Card><Card className="price-card"><span>Elite</span><strong>฿299 <small>/ month</small></strong><p>Connected-account and advanced intelligence roadmap.</p></Card></div></section>
    </main>
    <footer className="landing-footer"><strong>TRADOVIA</strong><span>Know your trading. Build your edge.</span><span>© Tradovia</span></footer>
  </div>;
}
